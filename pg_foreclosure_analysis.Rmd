---
title: "Foreclosure Data Analysis"
author: "Jasper Evans, Krehl Kasayan, Paul Kiefer, Pablo Suarez"
date: "2024-20-2024"
output: html_document
---

# Prince George's County Maryland Foreclosure Analysis

We will use Prince George's County's foreclosure dataset as the core of our project (https://data.princegeorgescountymd.gov/Urban-Planning/County-Foreclosures/mnie-hrv7/about_data). The dataset is maintained by the Prince George's County planning department and lists foreclosures by address between 2009 and 2024. 

## Data Setup

Load libraries.

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
options(scipen=999)
##install.packages("ggrepel")
##install.packages('ggthemes')
library(tidyverse)
library(lubridate)
library(janitor)
library(ggthemes)
library(tidycensus)
library(ggplot2)
library(ggrepel)
library(tigris)
library(sf)
library(ggmap)
```


The foreclosure dataset contains 71676 rows and 10 columns, including the street number, street name, city, state and zip code of each foreclosure.

```{r}
foreclosure_pg <- read_csv("datasets/pg_foreclosures.csv", guess_max = 71676)
```

We will eventually need to generate census tract ID numbers for each address, so we will create a new dataframe that excludes records with missing zip code values. This filters out roughly 1600 records.

```{r}
foreclosure_pg_census_tracts <- read_csv("datasets/pg_foreclosure_tract_geocodio.csv", guess_max = 71676) |>
  clean_names()
```

```{r}
tracts_unique <- length(unique(foreclosure_pg_census_tracts$census_tract_code))

print(tracts_unique)
```

```{r}
# Perform inner join based on the 'location' column
pg_foreclosures_with_tracts <- left_join(foreclosure_pg_census_tracts, foreclosure_pg, by = "location")

# Convert "submitteddate" column to date type
pg_foreclosures_with_tracts$submitteddate <- as.Date(pg_foreclosures_with_tracts$submitteddate, format = "%m/%d/%Y")

# Extract the year and month from "submitteddate"
pg_foreclosures_with_tracts$year <- year(pg_foreclosures_with_tracts$submitteddate)
pg_foreclosures_with_tracts$month <- month(pg_foreclosures_with_tracts$submitteddate)

# Create a year-month column for deduplication
pg_foreclosures_with_tracts$year_month <- as.Date(paste(pg_foreclosures_with_tracts$year, pg_foreclosures_with_tracts$month, "01", sep = "-"), format = "%Y-%m-%d")

# Print the dataset
print(foreclosure_pg_census_tracts)
```
```{r}
# To cast a wide net, we create one version that identifies rows with matching values in the "street_address", "city.y", "propertyid", and "year" columns -- columns that consistently match from entry to entry.

columns_wide <- c("street_address", "city.y", "propertyid", "year")

# To cast a narrower net, we create another version that identifies rows with matching values in the "street_address", "city.y", and "propertyid" columns, as well as the year_month column -- a column that may change within an otherwise matching set. 

columns_narrow <- c("street_address", "city.y", "propertyid", "year_month")

duplicates_logical_wide <- duplicated(pg_foreclosures_with_tracts[columns_wide], fromLast = TRUE) | duplicated(pg_foreclosures_with_tracts[columns_wide])

duplicates_logical_narrow <- duplicated(pg_foreclosures_with_tracts[columns_narrow], fromLast = TRUE) | duplicated(pg_foreclosures_with_tracts[columns_narrow])

# Subset the original dataset using the wide and narrow duplicate vectors to get the duplicate rows
duplicates_wide <- subset(pg_foreclosures_with_tracts, duplicates_logical_wide)

duplicates_narrow <- subset(pg_foreclosures_with_tracts, duplicates_logical_narrow)

# Arrange the rows to consolidate matching groups.
duplicates_wide <- arrange(duplicates_wide, street_address, city.y, propertyid, year)

duplicates_narrow <- arrange(duplicates_narrow, street_address, city.y, propertyid, year_month)

rows_to_keep_narrow <- distinct(duplicates_narrow, .keep_all = TRUE)
```

```{r}
missing <- anti_join(duplicates_narrow, duplicates_wide, by = c("propertyid"))
```

```{r}
rows_to_keep_narrow <- distinct(duplicates_narrow, .keep_all = TRUE)
```

```{r}
pg_foreclosures_filtered <- anti_join(pg_foreclosures_with_tracts, duplicates_wide, by = c("street_address", "city.y", "propertyid", "year_month"))
```

```{r}
pg_foreclosures_filtered <- bind_rows(pg_foreclosures_filtered, rows_to_keep_narrow)
```

```{r}
pg_foreclosures_filtered <- pg_foreclosures_filtered[!is.na(pg_foreclosures_filtered$census_tract_code), ]
```

```{r}
pg_foreclosures_filtered <- select(pg_foreclosures_filtered, -accuracy_score, -accuracy_type, -source, -full_fips_block, -metro_micro_statistical_area_name,-metro_micro_statistical_area_code, -metro_micro_statistical_area_type, -combined_statistical_area_name, -metropolitan_division_area_name, -metropolitan_division_area_code, -addressoccupied, -full_fips_block)
```

```{r}
pg_foreclosures_filtered <- pg_foreclosures_filtered |>
  filter(state.x == "MD")
```

```{r}
pg_foreclosures_per_tract <- pg_foreclosures_filtered |>
  group_by(census_tract_code) |>
  summarize(foreclosure_count = n())
```

```{r}
#Grab population by md tract
md_tract_pop <- get_acs(geography = "tract",
              variables = c(tract_pop = "B01003_001"),
              state = "MD",
              year = 2020,
              geometry = TRUE)

#Drop state ID # and county code prefixes from the tract ID # in the GEOID column.
```


```{r}
pg_tract_pop <- md_tract_pop[str_detect(md_tract_pop$NAME, "Prince George"), ]

pg_tract_pop$tract_number <- gsub('24033', '', pg_tract_pop$GEOID)
```

```{r}
pg_foreclosures_per_tract <- left_join(md_tract_pop, pg_foreclosures_per_tract, by=c("tract_number" = "census_tract_code")) |>
  #Rename variable
  rename(tract_pop = estimate) |>
  #Add notices per capita column
  mutate(foreclosure_pc_2020 = (foreclosure_count/tract_pop)*1000)
```

