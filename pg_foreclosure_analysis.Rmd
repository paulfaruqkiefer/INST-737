...
for (variable in variables_of_interest_subset_1) {
  ...
  predictions <- predict(model, newdata = pg_foreclosures_test, interval = "confidence", level = 0.95)
  
  # Calculate correlation between predicted and actual values
  correlation_test <- cor(predictions[, "fit"], pg_foreclosures_test$foreclosure_pc_2020)
  
  # Calculate mean square error
  mse_test <- mean((predictions[, "fit"] - pg_foreclosures_test$foreclosure_pc_2020)^2)
  
  # Plot predicted vs. actual values
  plot(pg_foreclosures_test$foreclosure_pc_2020, predictions[, "fit"], 
       main = paste("Predicted vs. Actual for", variable),
       xlab = "Actual Values", ylab = "Predicted Values", ylim = range(c(predictions[, "fit"], pg_foreclosures_test$foreclosure_pc_2020)))
...
for (variable in variables_of_interest_subset_2) {
  ...
  # Calculate correlation between predicted and actual values
  correlation_test <- cor(predictions[, 1], pg_foreclosures_test$foreclosure_pc_2020)
  
  # Calculate mean square error
  mse_test <- mean((predictions[, 1] - pg_foreclosures_test$foreclosure_pc_2020)^2)
  
  # Plot predicted vs. actual values with confidence bands
  plot(pg_foreclosures_test$foreclosure_pc_2020, predictions[, 1], 
       main = paste("Predicted vs. Actual for", variable),
...
for (variable in variables_of_interest_subset_3) {
 ...
 plot(pg_foreclosures_test$foreclosure_pc_2020, predictions[, 1], 
       main = paste("Predicted vs. Actual for", variable),
       xlab = "Actual Values", ylab = "Predicted Values", ylim = range(c(predictions[, 2], predictions[, 3])))
  abline(0, 1, col = "red") # Add a diagonal line for reference
...
for (i in 1:length(variables_of_interest_subset_1)) {
    ...
    X_train <- as.matrix(pg_foreclosures_train[, variable, drop = FALSE])  
    ...
    if (ncol(X_train) == 1) {
        X_train <- cbind(1, X_train)  # Include an intercept term
    }
    ...
    prediction <- predict(lasso_model, newx = X_test, s = lambda, type = "response")
    
    # Calculate standard errors for prediction intervals
    X_test_augmented <- cbind(1, X_test)  # Add intercept for standard errors
    se <- sqrt(rowSums(predict(lasso_model, newx = X_test_augmented, s = lambda, type = "nonzero")^2))
    prediction_interval <- qnorm(0.975) * se
    
    # Plot predicted vs. actual values with prediction bands
    plot(pg_foreclosures_test$foreclosure_pc_2020, prediction, 
         main = paste("Predicted vs. Actual for", variable),
         xlab = "Actual Values", ylab = "Predicted Values", 
         ylim = range(c(prediction, pg_foreclosures_test$foreclosure_pc_2020))) 
    abline(0, 1, col = "red") # Add a diagonal line for reference
    
    # Add prediction bands
    matlines(x = rep(pg_foreclosures_test$foreclosure_pc_2020, 3), 
             y = c(prediction - prediction_interval, prediction, prediction + prediction_interval), 
             col = "green", lty = c(1, 2, 1))
    
    # Calculate correlation between predicted and actual values
    correlation_test <- cor(prediction, pg_foreclosures_test$foreclosure_pc_2020)
    
    # Calculate mean square error
    mse_test <- mean((prediction - pg_foreclosures_test$foreclosure_pc_2020)^2)
...
We need to repeat our final step multiple times with different samples to see if we reach the same result.

We can do this using a loop:

```{r}
# Set the number of repetitions
num_repetitions <- 5  # You can adjust this number as needed

# Loop through repetitions
for (i in 1:num_repetitions) {
    # Split the data into training and testing sets
    set.seed(i)  # Use different seed for each iteration for reproducibility
    train_indices <- createDataPartition(pg_foreclosures_per_tract$foreclosure_pc_2020, p = 0.8, list = FALSE)
    ...
    # Use the trained model to predict on the testing data
    predicted_values <- predict(lasso_model, newx = as.matrix(test_data[, test_variables_3]),
                                s = "lambda.min")
    ...
    # Identify the most predictive features
    non_zero_coefficients <- coefficients[-1]  # Exclude intercept
    top_predictive_features <- names(non_zero_coefficients[non_zero_coefficients != 0])
