...
for (variable in variables_of_interest_subset_1) {
  ...
  # Plot residuals
  plot(residuals, main = paste("Residuals for", variable), xlab = "Index", ylab = "Residuals")
}
```
Subset 2:
```{r}
...
for (variable in variables_of_interest_subset_3) {
  # Perform linear regression
  model <- lm(foreclosure_pc_2020 ~ ., data = pg_foreclosures_per_tract[, c(variable, "foreclosure_pc_2020")])
...
lambda <- 0.1  # You can adjust this parameter to control the amount of regularization

for (variable in variables_of_interest) {
  tryCatch({
    # Prepare the data
    X <- model.matrix(foreclosure_pc_2020 ~ ., data = pg_foreclosures_train[, c(variable, "foreclosure_pc_2020")])[, -1] # Exclude intercept
