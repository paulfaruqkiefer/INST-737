...
```{r}
par(mfrow=c(3, 4)) # Adjust rows and columns according to your preference
...
for (variable in variables_of_interest_subset_1) {
  ...
  predictions <- predict(model, newdata = pg_foreclosures_test)
...
```
We can also visualize these correlations using a scatter plot.

```{r}
# Loop through each independent variable
for (variable in variables_of_interest) {
  # Create a new plotting window for each scatterplot
  plot.new()
  
  # Create scatterplot
  plot(pg_foreclosures_per_tract[[variable]], pg_foreclosures_per_tract$foreclosure_pc_2020,
       xlab = variable, ylab = "foreclosure_pc_2020", main = paste("Scatterplot of", variable))
}
```
...
To answer the core questions, we also need to assess the residuals.

```{r}
# Loop through each independent variable
for (variable in variables_of_interest) {
  # Fit linear regression model
  lm_model <- lm(foreclosure_pc_2020 ~ pg_foreclosures_per_tract[[variable]], data = pg_foreclosures_per_tract)
  
  # Print results
  cat("Variable:", variable, "\n")
  cat("Intercept:", coef(lm_model)[1], "\n")
  cat("Coefficient:", coef(lm_model)[2], "\n")
  cat("Is it a predictive feature?:", ifelse(summary(lm_model)$coefficients[2, "Pr(>|t|)"] < 0.05, "Yes", "No"), "\n")
  cat("\n")
}
...
# 1. Linearity: You can check this using scatterplots of each independent variable against the dependent variable.
# 2. Independence: You can check this using residual plots or Durbin-Watson test.
# 3. Homoscedasticity: You can check this using residual plots.
# 4. Normality of residuals: You can check this using a histogram or Q-Q plot of residuals.

# For example:
# Plot residuals vs fitted values
plot(residuals ~ fitted(lm_model), xlab = "Fitted values", ylab = "Residuals", main = "Residuals vs Fitted")
abline(h = 0, col = "red")  # Add a horizontal line at y = 0
```

If we plot the residuals as a histogram, we see that they are (roughly) normally distributed.
```{r}
...
```{r}
# Selecting the variables of interest (excluding foreclosure_pc_2020)
test_variables_1 <- c("avg_bed", "tract_homevalue_2020", "tract_medage_2020", "tract_medincome_2020",   "foreclosure_pc_2010", "pct_built_2020_later", "pct_built_2010_2019", "pct_built_2000_2009",  "pct_built_1990_1999", "pct_built_1980_1989", "pct_built_1970_1979", "pct_built_pre_1960", "poverty_2010", "nhwhite_2020", "mortgaged_2010", "mortgage_change_2010_2015", "poverty_change_2010_2020","nhwhite_change_2010_2020","medincome_change_2010_2015", "medincome_change_2015_2020", "medincome_change_2010_2020", "pop_change_pct")
```
...
lm_model <- lm(foreclosure_pc_2020 ~ ., data = train_data[, c("foreclosure_pc_2020", test_variables_1)])

# Use the trained model to predict on the testing data
predicted_values <- predict(lm_model, newdata = test_data[, test_variables_1])
