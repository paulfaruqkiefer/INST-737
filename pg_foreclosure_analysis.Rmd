...
```{r}
md_medhouseholdincome_2020_tract <- get_acs(geography = "zcta",
...
We start with the number of housing units per tract:

```{r}
# Left join the pg_tract_pop_2020 data from the ACS with the pg_foreclosures_per_tract dataset.
pg_foreclosures_per_tract <- left_join(pg_tract_pop_2010, pg_foreclosures_per_tract, by=c("tract_number" = "census_tract_code")) |>
  #Rename the "estimate" variable
  rename(tract_housing_units_2010 = estimate) |>
  #Remove extraneous columns.
  select(-NAME, -variable) 
```
